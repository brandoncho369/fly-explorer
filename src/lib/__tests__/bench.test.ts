...
describe("committed snapshot", () => {
  ...
  it("has 5 core + 15 hard tasks; every run's tasks are known tasks and every run covers the core tier", () => {
    expect(snap.tasks.filter((t) => t.tier === "core")).toHaveLength(5);
    expect(snap.tasks.filter((t) => t.tier === "hard")).toHaveLength(15);
