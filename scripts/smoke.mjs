...
await p.waitForFunction(() => document.body.innerText.includes("3,792"), null, { timeout: 30000 });
...
for (const w of [1300, 400]) {
  ...
  check((await q.locator("#tasks article").count()) === 34, `/bench @${w}px: 34 task cards`);
...
// permalinks (ROADMAP item 53): the address bar holds the experiment; names the dataset lacks are banner-ed, never substituted
{
  ...
  await p2.waitForFunction(() => document.body.innerText.includes("3,792"), null, { timeout: 30000 });
